...
function Field({ label, value, onChange, error, type = 'text', required = false }) { return <label className="scuml-field">{label}{required && ' *'}<input type={type} value={value || ''} onInput={(event) => onChange(event.currentTarget.value)} />{error && <small className="scuml-error">{error}</small>}</label>; }
