...
function NINVerificationPage() {
  ...
  const submit = async (event) => {
    ...
    try {
      if (editMode) {
        const session = readApplicationEditSession();
        const { error } = await updateApplicationForEdit(reference, session?.application?.email || form.email, { nin: form.nin, phone: form.phone, surname: form.surname, first_name: form.firstName, date_of_birth: form.dateOfBirth || null, email: form.email, address: form.address }, session?.application?.id, 'Client');
