...
function Field({ label, value, onChange, error, type = 'text', required = false }) { return <label className="nin-field">{label}{required && ' *'}<input type={type} value={value || ''} onInput={(event) => onChange(event.currentTarget.value)} />{error && <small className="nin-error">{error}</small>}</label>; }
