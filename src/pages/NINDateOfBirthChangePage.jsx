...
function NINDateOfBirthChangePage() {
  ...
  const updateLocation = (field, value) => setForm((current) => ({
    ...current,
    [field]: value,
    ...(field.endsWith('State') || field === 'stateOfOrigin' || field === 'stateOfBirth' || field === 'stateOfResidence'
      ? { [field.replace('State', 'Lga').replace('stateOfOrigin', 'lgaOfOrigin').replace('stateOfBirth', 'lgaOfBirth').replace('stateOfResidence', 'lgaOfResidence')]: '' }
      : {}),
  }));
  const submit = async (event) => {
    ...
    try {
      ...
      setError(submitError.message || 'Unable to submit the date-change request. Please try again.');
    } finally {
      setBusy(false);
    }
  };
  if (submitted) return <PaymentSubmissionCard title="Date of birth change request submitted" reference={submitted} />;
  ...
  function fields(items) { return items.map(([field, label, type = 'text', required = true, options]) => { const isState = field.endsWith('State') || ['stateOfOrigin', 'stateOfBirth', 'stateOfResidence'].includes(field); const isLga = field.endsWith('Lga') || field === 'lgaOfOrigin'; const stateField = field === 'lgaOfOrigin' ? 'stateOfOrigin' : field.replace('Lga', 'State'); const fieldOptions = isState ? states : isLga ? lgasByState[form[stateField]] || [] : options; return <Field key={field} label={label} type={type} value={form[field]} options={fieldOptions} onChange={(value) => (isState ? updateLocation(field, value) : update(field, value))} required={required} />; }); }
}
