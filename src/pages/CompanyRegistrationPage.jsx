...
function CompanyRegistrationPage() {
  ...
  const updatePerson = (group, index, field, value) => setForm((current) => {
    ...
    if (field === '__remove' && Array.isArray(current[stateKey]) && current[stateKey].length > 1) return { ...current, [stateKey]: current[stateKey].filter((_, personIndex) => personIndex !== index) };
    return {
      ...current,
  ...
  return (
    <main className="company-page">
      <section className="company-card">
        ...
        <form onSubmit={submit} onKeyDown={(event) => { if (event.key === 'Enter' && event.target.tagName !== 'TEXTAREA') event.preventDefault(); }}>
...
function PaymentView({ reference, paymentSlip, setPaymentSlip, submitPayment, paymentSubmitted, navigate }) { return <PaymentSubmissionCard title="Complete CAC Company Registration Payment" reference={reference} file={paymentSlip} onFileChange={setPaymentSlip} onSubmit={submitPayment} paymentSubmitted={paymentSubmitted} onComplete={() => navigate('/')} />; }
