    ...
    <main className="company-page">
      <section className="company-card">
        ...
        <form onSubmit={submit}>
          {step === 1 && <Step title="Company and entity details"><div className="company-grid"><Field label="Proposed company name 1 (must end with LTD or LIMITED)" value={form.proposedName1} error={errors.proposedName1} onChange={(value) => update('proposedName1', value)} required /><Field label="Proposed company name 2" value={form.proposedName2} onChange={(value) => update('proposedName2', value)} /><Field label="Company email" type="email" value={form.email} error={errors.email} onChange={(value) => update('email', value)} required /><Field label="Company phone" value={form.phone} error={errors.phone} onChange={(value) => update('phone', value)} required /><Field label="State" value={form.state} onChange={(value) => update('state', value)} required /><Field label="LGA" value={form.lga} onChange={(value) => update('lga', value)} required /><Field label="Town / city / village" value={form.town} onChange={(value) => update('town', value)} required /><Field label="House number" value={form.houseNumber} onChange={(value) => update('houseNumber', value)} /><Field label="Street name" value={form.streetName} onChange={(value) => update('streetName', value)} required /></div>{errors.address && <p className="company-error">{errors.address}</p>}</Step>}
...
function Field({ label, value, onChange, error, type = 'text', required = false }) { return <label className="company-field">{label}{required && ' *'}<input type={type} value={value || ''} onChange={(event) => onChange(event.target.value)} />{error && <small className="company-error">{error}</small>}</label>; }
