...
function PaymentView({ reference, paymentSlip, setPaymentSlip, submitPayment, paymentSubmitted, navigate }) { return <main className="company-page"><section className="company-card company-payment"><img src="/logo/logo2.jpeg" alt="Pernest Digital Services" /><p className="company-kicker">Application received</p><h1>Complete CAC Company Registration Payment</h1><p>Reference: <strong>{reference}</strong></p><div className="company-account"><strong>Pay the agreed service charge</strong><span>Account: 8130801666</span><span>Bank: Opay</span><span>Account name: ERNEST EMMANUEL OSUNG</span></div><p>Part payment is accepted, but complete payment is required before certificate issuance.</p><label className="company-file">Upload bank transfer slip<input type="file" accept="image/*,.pdf" onChange={(event) => setPaymentSlip(event.target.files[0])} />{paymentSlip && <small>{paymentSlip.name}</small>}</label><button className="company-primary" type="button" onClick={submitPayment} disabled={!paymentSlip || paymentSubmitted}>{paymentSubmitted ? 'Slip submitted for review' : 'Submit payment slip'}</button>{paymentSubmitted && <button className="company-secondary" type="button" onClick={() => navigate('/')}>Return to website</button>}</section></main>; }
